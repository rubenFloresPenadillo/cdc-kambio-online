...
//    	codOperClie = (Integer) sesion.getAttribute("codOperClie");
    	...
    	if(indCompleDatos.equals(NumerosType.NUMERO_MINIMO_CERO.getValor())) {
    		...
    		if (indFueraDeHorario.equals(NumerosType.INDICADOR_POSITIVO_UNO.getValor())) {
        		...
        		mensajeHorarioAtencion = "Horario de atenci�n: Lunes a Viernes de "+horaInicioLaV.format(pattern)+" a "+horaFinalLaV.format(pattern)
        		+" y S�bados de "+horaInicioSab.format(pattern)+" a "+horaFinalSab.format(pattern);
...
//    			        	cuentaBancoAdminDetalle.getTpBanco().setCodBanc(operacionClienteFormulario.getTpBanco().getCodBanc());
    ...
    public void validarSeleccionCuenta() {
    	...
    	if(operacionClienteFormulario.getTpCuentBancoByCodCuenBancClieOrig().getCodCuenBanc() == null) {
    		result = "Debes seleccionar la cuenta de donde env�as tu dinero.";
...
//    	
//    	
//    	String asunto = "Operaci�n con c�digo "+operacionClienteFormulario.getCodUnicOperClie()+ " registrada, pendiente de verificaci�n.";
...
//        		enviarCorreoRegistroOperacion();
    		...
    		resultadoProcesoError = "El codigo de operación bancaria es obligatorio, por favor verifique";
