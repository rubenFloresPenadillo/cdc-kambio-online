...
function funcionCargaEstiloActividad(){
	...
	$("#idTablaPrincipal tr").each(function() {
		...
		var item = $(this).find("td:eq(5)");
	       ...
	       if(item.text() == 'Activada') {
	    	   item.addClass("claseIndicadorActivo");
	       }else if(item.text() == 'Eliminada' || item.text() == 'Bloqueada'){
	    	   item.addClass("claseIndicadorInactivo");
	       }else if (item.text() == 'Registrada'){
	    	   item.addClass("claseIndicadorRegistrado");
	       }
...
function operacionGuardarUsuarioExitosa(){
